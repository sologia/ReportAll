...
// permite activar una �nica selecci�n por clic en el mapa
...
// funci�n que env�a datos al backend (usa FormData para adjuntar imagen si hay)
// mantiene compatibilidad con la versi�n previa: usa la geometr�a actualmente dibujada
export async function sendReport(formFields, fileInput) {
  const wkt = getGeometryWKT();
  if (!wkt) {
    throw new Error('Dibuja la geometr�a en el mapa antes de enviar.');
  }
  ...
  fd.append('GeoM', wkt); // el backend esperar� WKT
