import { poolPromise, sql } from '../config/db.js';

// GET /api/clients
export async function getAll(req, res, next) {
    try {
        const pool = await poolPromise;
        const result = await pool.request().query('SELECT  FirstName, SecondName, FirstLastName, SecondLastName, Numero_NIC FROM Clients');
        res.json(result.recordset);
    } catch (err) {
        next(err);
    }
}

// GET /api/clients/:id
export async function getById(req, res, next) {
    try {
        const id = parseInt(req.params.id, 10);
        if (Number.isNaN(id)) return res.status(400).json({ message: 'Invalid id' });

        const pool = await poolPromise;
        const result = await pool.request()
            .input('id', sql.Int, id)
            .query('SELECT FirstName_Client, SecondName_Client, FirstLastName_Client, SecondLastName_Client, Numero_NIC FROM Clients WHERE Client_ID = @id');

        const row = result.recordset[0];
        if (!row) return res.status(404).json({ message: 'Not found' });
        res.json(row);
    } catch (err) {
        next(err);
    }
}

// POST /api/clients
export async function create(req, res, next) {
    try {
        const { FirstName, SecondName, FirstLastName, SecondLastName, Numero_NIC } = req.body;
        const pool = await poolPromise;

        const request = pool.request()
            .input('FirstName', sql.NVarChar(100), FirstName)
            .input('SecondName', sql.NVarChar(100), SecondName)
            .input('FirstLastName', sql.NVarChar(100), FirstLastName)
            .input('SecondLastName', sql.NVarChar(100), SecondLastName)
            .input('Numero_NIC', sql.NVarChar(50), Numero_NIC);

        const sqlQuery = `
            INSERT INTO Clients (FirstName_Client, SecondName_Client, FirstLastName_Client, SecondLastName_Client, Numero_NIC)
            OUTPUT INSERTED.Client_ID, INSERTED.FirstName_Client, INSERTED.SecondName_Client, INSERTED.FirstLastName_Client, INSERTED.SecondLastName_Client, INSERTED.Numero_NIC
            VALUES (@FirstName, @SecondName, @FirstLastName, @SecondLastName, @Numero_NIC);
        `;

        const result = await request.query(sqlQuery);
        res.status(201).json(result.recordset && result.recordset[0] ? result.recordset[0] : {});
    } catch (err) {
        next(err);
    }
}

// PUT /api/clients/:id
export async function update(req, res, next) {
    try {
        const id = parseInt(req.params.id, 10);
        if (Number.isNaN(id)) return res.status(400).json({ message: 'Invalid id' });

        const { FirstName, SecondName, FirstLastName, SecondLastName, Numero_NIC } = req.body;
        const pool = await poolPromise;
        const request = pool.request().input('id', sql.Int, id);

        let setParts = [];
        if (FirstName !== undefined) {
            setParts.push('FirstName_Client = @FirstName, ');
            request.input('FirstName', sql.NVarChar(100), FirstName);
        }
        if (SecondName !== undefined) {
            setParts.push('SecondName_Client = @SecondName, ');
            request.input('SecondName', sql.NVarChar(100), SecondName);
        }
        if (FirstLastName !== undefined) {
            setParts.push('FirstLastName_Client = @FirstLastName, ');
            request.input('FirstLastName', sql.NVarChar(100), FirstLastName);
        }
        if (SecondLastName !== undefined) {
            setParts.push('SecondLastName_Client = @SecondLastName, ');
            request.input('SecondLastName', sql.NVarChar(100), SecondLastName);
        }
        if (Numero_NIC !== undefined) {
            setParts.push('Numero_NIC = @Numero_NIC,  ');
            request.input('Numero_NIC', sql.NVarChar(50), Numero_NIC);
        }

        if (setParts.length === 0) return res.status(400).json({ message: 'No updatable fields provided' });

        const setClause = setParts.join(', ');
        const sqlQuery = `
            UPDATE Clients
            SET ${setClause}
            WHERE Client_ID = @id;

    
        `;

        const result = await request.query(sqlQuery);
        const updated = result.recordset && result.recordset[0] ? result.recordset[0] : null;
        if (!updated) return res.status(404).json({ message: 'Not found' });
        res.json(updated);
    } catch (err) {
        next(err);
    }
}

// DELETE /api/clients/:id
export async function remove(req, res, next) {
    try {
        const id = parseInt(req.params.id, 10);
        if (Number.isNaN(id)) return res.status(400).json({ message: 'Invalid id' });

        const pool = await poolPromise;
        await pool.request()
            .input('id', sql.Int, id)
            .query('DELETE FROM Clients WHERE Client_ID = @id');
        res.status(204).end();
    } catch (err) {
        next(err);
    }
}